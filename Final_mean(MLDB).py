...
findspark.init()
import pyspark
from pyspark.sql.functions import col,regexp_replace,when,row_number, lit,udf
findspark.find()
...
data = spark.read.csv('D:\cov_ass.csv', header=True, inferSchema=True)


# In[7]:


#finding the countries with the most and the least number of covid cases
#total_cases = data.select("Country/Region","11/23/21").groupBy("Country/Region").sum()
#total_cases_pd = qr.toPandas()
#total_cases_pd.sort_values('sum(11/23/21)',ascending=True).head(10)
